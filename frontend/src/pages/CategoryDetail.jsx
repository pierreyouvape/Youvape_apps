...
const CategoryDetail = () => {
  ...
  const { params, bar, filterKey } = useDetailFilters('brand', { category: categoryName });

  useEffect(() => {
    // Les produits déjà chargés l'ont été pour l'ancienne période / l'ancienne marque
  ...
  const fetchCategoryData = async () => {
    setLoading(true);
    try {
      const response = await axios.get(`${API_URL}/categories/${encodeURIComponent(categoryName)}`, { params });
  ...
  const fetchSubCategoryProducts = async (subCategoryName) => {
    ...
    try {
      const response = await axios.get(`${API_URL}/categories/sub-categories/${encodeURIComponent(subCategoryName)}`, { params });
      if (response.data.success) {
    ...
    <main className="main-scroll" style={{ flex: 1, minWidth: 0, overflowY: 'auto', height: '100vh', display: 'flex', flexDirection: 'column', backgroundColor: '#f5f5f5' }}>
      ...
      <div style={{ flex: 1, maxWidth: '1600px', margin: '30px auto', padding: '0 20px', width: '100%' }}>
        ...
        <div style={{ backgroundColor: '#fff', padding: '30px', borderRadius: '12px', boxShadow: '0 2px 8px rgba(0,0,0,0.05)', marginBottom: '30px' }}>
          <h1 style={{ margin: '0 0 8px 0', color: '#135E84', fontSize: '28px' }}>{category.category}</h1>
          {bar}

...
const SubBrandDetail = () => {
  const { subBrandName } = useParams();
  ...
  const fetchSubBrandData = async () => {
    ...
    try {
      ...
      console.error('Error fetching sub-brand:', err);
    } finally {
      setLoading(false);
    }
  };
  ...
  if (!subBrand) {
    return (
      <div style={{ minHeight: '100vh', display: 'flex', flexDirection: 'column', backgroundColor: '#f5f5f5' }}>
        ...
        <div style={{ flex: 1, display: 'flex', justifyContent: 'center', alignItems: 'center' }}>
          <div style={{ fontSize: '18px', color: '#666' }}>Sous-marque non trouvee</div>
        </div>
      </div>
    );
  }
      ...
      <div style={{ flex: 1, maxWidth: '1600px', margin: '30px auto', padding: '0 20px', width: '100%' }}>
        ...
        <div style={{ backgroundColor: '#fff', padding: '30px', borderRadius: '12px', boxShadow: '0 2px 8px rgba(0,0,0,0.05)', marginBottom: '30px' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: '15px', marginBottom: '20px' }}>
            {subBrand.brand && (
              <span
                onClick={() => navigate(`/brands/${encodeURIComponent(subBrand.brand)}`)}
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(180px, 1fr))', gap: '15px' }}>
            <div style={{ backgroundColor: '#f8f9fa', padding: '15px', borderRadius: '8px' }}>
              ...
              <div style={{ fontSize: '24px', fontWeight: '700', color: '#135E84' }}>{formatNumber(subBrand.stats.product_count)}</div>
            </div>
            <div style={{ backgroundColor: '#f8f9fa', padding: '15px', borderRadius: '8px' }}>
              <div style={{ fontSize: '12px', color: '#666', textTransform: 'uppercase' }}>Quantite vendue</div>
              <div style={{ fontSize: '24px', fontWeight: '700', color: '#333' }}>{formatNumber(subBrand.stats.qty_sold)}</div>
            </div>
            <div style={{ backgroundColor: '#f8f9fa', padding: '15px', borderRadius: '8px' }}>
              <div style={{ fontSize: '12px', color: '#666', textTransform: 'uppercase' }}>CA TTC</div>
              <div style={{ fontSize: '24px', fontWeight: '700', color: '#28a745' }}>{formatCurrency(subBrand.stats.ca_ttc)}</div>
            </div>
            <div style={{ backgroundColor: '#f8f9fa', padding: '15px', borderRadius: '8px' }}>
              <div style={{ fontSize: '12px', color: '#666', textTransform: 'uppercase' }}>CA HT</div>
              <div style={{ fontSize: '24px', fontWeight: '700', color: '#333' }}>{formatCurrency(subBrand.stats.ca_ht)}</div>
            </div>
            <div style={{ backgroundColor: '#f8f9fa', padding: '15px', borderRadius: '8px' }}>
              <div style={{ fontSize: '12px', color: '#666', textTransform: 'uppercase' }}>Marge HT</div>
              <div style={{ fontSize: '24px', fontWeight: '700', color: subBrand.stats.margin_ht >= 0 ? '#28a745' : '#dc3545' }}>{formatCurrency(subBrand.stats.margin_ht)}</div>
            </div>
            <div style={{ backgroundColor: '#f8f9fa', padding: '15px', borderRadius: '8px' }}>
              <div style={{ fontSize: '12px', color: '#666', textTransform: 'uppercase' }}>% Marge</div>
              <div style={{ fontSize: '24px', fontWeight: '700', color: subBrand.stats.margin_percent >= 0 ? '#28a745' : '#dc3545' }}>{formatPercent(subBrand.stats.margin_percent)}</div>
            </div>
          </div>
        </div>
        ...
        {/* Products Table */}
        <div>
          <h2 style={{ color: '#333', marginBottom: '15px' }}>Produits ({subBrand.products?.length || 0})</h2>
            ...
            <table style={{ width: '100%', borderCollapse: 'collapse', minWidth: '900px' }}>
              <thead>
                <tr style={{ backgroundColor: '#f8f9fa', borderBottom: '2px solid #e0e0e0' }}>
                  ...
                  <th style={{ textAlign: 'right', padding: '15px', fontSize: '14px', fontWeight: '600', color: '#333' }}>Prix TTC</th>
        ...
        {/* Empty state */}
        {(!subBrand.products || subBrand.products.length === 0) && (
          <div style={{ textAlign: 'center', padding: '50px', backgroundColor: '#fff', borderRadius: '12px', boxShadow: '0 2px 8px rgba(0,0,0,0.05)' }}>
            <div style={{ fontSize: '18px', color: '#666' }}>Aucun produit dans cette sous-marque</div>
          </div>
        )}
      </div>

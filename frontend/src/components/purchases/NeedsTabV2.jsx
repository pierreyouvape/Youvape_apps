...
// V2 : clé localStorage distincte de la V1 pour ne pas interférer avec
// l'app "Gestion d'achat" existante (le patron garde ses réglages intacts).
const STORAGE_KEY = 'purchases_needs_filters_v2';
...
// V2 : seuil d'alerte — durée minimale que le stock doit encore couvrir.
// En-dessous, le produit s'affiche et une proposition est calculée.
// C'est le DÉCLENCHEUR, distinct de la couverture (la CIBLE de réassort).
const ALERT_OPTIONS = [
  { value: 0.25, label: '7 jours' },
  { value: 0.5, label: '15 jours' },
  { value: 1, label: '1 mois' },
...
const computeProductNeeds = (product, periodDays, coverageMonths, isCustomPeriod, analysisStartDate, analysisEndDate, periodUnit, alertMonths) => {
  ...
  // V2 : garde-fou — la couverture ne peut pas être inférieure au seuil d'alerte,
  // sinon on proposerait de remonter à un niveau déjà sous l'alerte → re-déclenchement.
  ...
  if (isCustomPeriod && analysisStartDate && analysisEndDate) {
    ...
    // N derniers mois — inclure le mois en cours (temps réel)
    const now = new Date();
...
const NeedsTabV2 = ({ token, onCompactChange }) => {
  ...
  // Seuil d'alerte (V2) : déclencheur d'affichage, découplé de la couverture.
  // Défaut 1 mois — on n'affiche que les produits qui ne tiennent pas 1 mois.
  const [alertMonths, setAlertMonths] = useState(savedFilters?.alertMonths ?? 1);

  // Couverture cible (quantité de réassort)
  const [coverageMonths, setCoverageMonths] = useState(savedFilters?.coverageMonths || 2);
  ...
  }, [supplierId, brandFilter, analysisPeriodType, analysisPeriod, analysisPeriodUnit, analysisStartDate, analysisEndDate, alertMonths, coverageMonths, withSalesOnly, zeroStockState]);
  ...
  const computedProducts = useMemo(() => {
    return allProducts.map(p => {
      const needs = computeProductNeeds(
        ...
        isCustomPeriod, analysisStartDate, analysisEndDate, analysisPeriodUnit, alertMonths
      );
      return { ...p, ...needs };
    });
  ...
  useEffect(() => {
    setPage(1);
  }, [supplierId, brandFilter, withSalesOnly, zeroStockState, effectivePeriodDays, alertMonths, coverageMonths, analysisStartDate, analysisEndDate, pageSize]);
      ...
      <div className="purchases-card" style={{ flexShrink: 0 }}>
        ...
        <div className="filters-bar">
          ...
          <div className="filter-group">
            <label>
              Couverture cible
              {coverageMonths < alertMonths && (
                <span
                  title="La couverture ne peut pas être inférieure au seuil d'alerte. Elle est ramenée au niveau de l'alerte pour éviter des recommandations qui se re-déclenchent aussitôt."
                  style={{ color: '#d97706', marginLeft: 6, cursor: 'help' }}
                >
                  ⚠ ajustée à {alertMonths < 1 ? `${Math.round(alertMonths * 30)} j` : `${alertMonths} mois`}
                </span>
              )}
            </label>

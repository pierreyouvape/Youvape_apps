...
const mailgunService = {
  ...
  // `inline` : images embarquées dans le message et référencées par `cid:` dans
  // le HTML. À privilégier sur une URL distante, que les messageries bloquent
  // par défaut quand l'expéditeur n'est pas connu du destinataire.
  sendReply: async ({ to, subject, ticketId, bodyText, bodyHtml, attachments, inline }) => {
    try {
